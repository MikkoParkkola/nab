...
impl SiteProvider for StackOverflowProvider {
    ...
    async fn extract(
        ...
        let api_url = format!(
            "https://api.stackexchange.com/2.3/questions/{question_id}?site=stackoverflow&filter=!6WPIomnMOOD*e&order=desc&sort=votes"
        );
...
async fn fetch_answers(client: &AcceleratedClient, question_id: &str) -> Result<Vec<SEAnswer>> {
    let api_url = format!(
        "https://api.stackexchange.com/2.3/questions/{question_id}/answers?site=stackoverflow&filter=!6WPIomnMOOD*e&order=desc&sort=votes&pagesize=3"
    );
